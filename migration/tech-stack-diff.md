...
Companion to [OpenELIS-Go-Migration-Plan.md](OpenELIS-Go-Migration-Plan.md) and
...
| Schema migration | **Liquibase 4.8.0** (277 changesets) | **Liquibase kept** during coexistence; later `goose`/`golang-migrate` | Liquibase stays the single schema owner until Java retires. |
...
Measured from the running `openelisglobal-webapp` container
...
- **TLS/keystore handled by nginx, not the app.** The Go binary drops the
  entire Tomcat SSL/OpenSSL/APR stack.
